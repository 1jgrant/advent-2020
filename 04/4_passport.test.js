const { passCheck } = require("../4_passport");
...
describe.skip("passCheck", () => {
  test("should correctly check one passport", () => {
    const input = `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
byr:1937 iyr:2017 cid:147 hgt:183cm
`;
    expect(passCheck(input)).toBe(1);
  });
  test("should find no. of valid passports in test data", () => {
    expect(passCheck(testData)).toBe(2);
  });
  test("should find no. of valid passports in actual data", () => {
    expect(passCheck(data)).toBe(210);
  });
});
...
`;
  ...
  test("should identify valid passwords", () => {
    expect(passCheck(valid)).toBe(4);
  });

  test("should identify invalid passwords", () => {
    expect(passCheck(invalid)).toBe(0);
  });

  test("should identify number of valid passwords in the actual data", () => {
    expect(passCheck(data)).toBe(131);
  });
});
